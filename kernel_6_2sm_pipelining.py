...
- five-stage circular A/B shared-memory pipeline
...
def _ensure_cuda_bindings_driver():
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "kernel6_2sm_pipelining requires the CuTeDSL runtime, "
...
def _get_cutedsl_launcher():
    ...
    class SharedStorage:
        ...
        # The TMA and MMA warps independently advance through these five stages.
    ...
    def kernel(
        ...
        # Cluster-aware full/empty barriers make the five A/B buffers a circular
        ...
        # circular buffer until all five stages are in flight, then waits only
        ...
        # Four output warps retain kernel 5's epilogue. The one-stage C buffer
        # and producer_acquire after every store deliberately serialize output;
        # kernel 7's double-buffered write-out is not implemented here.
...
def run_matmul_local(
    ...
    """Execute the CuTeDSL pipelined pair-MMA GEMM on Blackwell."""
    try:
    ...
    if requested_dtype_name not in (DEFAULT_DTYPE, "float16", "float32"):
        raise ValueError(
            ...
            f"kernel6_2sm_pipelining only supports {DEFAULT_DTYPE} inputs."
        )
    ...
    if err != cu_driver.CUresult.CUDA_SUCCESS or device_count < 1:
        raise RuntimeError("A GPU is required to run kernel6_2sm_pipelining.")
    ...
    return {
        ...
        "implementation": "cutedsl_blackwell_5stage_tma_2sm_mma_warp_specialized",
        "operation": "cute.gemm",
        ...
        "ab_stages": AB_STAGES,
        "schedule": "warp_specialized_5stage_tma_pair_mma_pipeline",
        "epilogue_stages": EPI_STAGES,
        "epilogue": "single_buffered_stmatrix_to_smem_tma_store",
