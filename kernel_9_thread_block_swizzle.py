...
writeout path for C.  Apart from the scheduler mapping, the implementation intentionally
keeps kernel 8 unchanged and avoids layering on additional optimizations.
"""
...
def _ensure_cuda_bindings_driver() -> None:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "kernel9_thread_block_swizzle requires cuda.bindings. Install cuda-bindings "
...
def _get_cutedsl_launcher():
    ...
    def _matmul_persistent_kernel(
        ...
        swizzle_n_bound = swizzle_n_size * BLOCK_SWIZZLE_SIZE
        ...
        if warp_idx == TMA_WARP_ID:
            ...
            tma_swizzled_cluster_m = cutlass.Int32(
                cutlass.select_(
                    ...
                    cutlass.select_(
                        tma_cluster_n < swizzle_n_bound,
                        tma_swizzled_m_candidate,
                        tma_cluster_m,
                    ),
                    tma_cluster_m,
                )
            )
            tma_swizzled_cluster_n = cutlass.Int32(
                cutlass.select_(
                    tma_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        tma_cluster_n < swizzle_n_bound,
            ...
            tma_work_n = (
                tma_swizzled_cluster_n * CLUSTER_SHAPE_MN[1] + cta_n_in_cluster
            )
            ...
            while tma_work_valid:
                ...
                tma_work_n = (
                    tma_swizzled_cluster_n * CLUSTER_SHAPE_MN[1]
                    + cta_n_in_cluster
                )
        ...
        if warp_idx == SCHEDULER_WARP_ID and is_first_cta_in_cluster:
            # The scheduler warp consumes the same remapped coordinates as the
            # load, MMA, and epilogue warps so all pipeline roles stay aligned.
            ...
            scheduler_m_local = (
                ...
                + swizzle_m_size
                * (scheduler_cluster_n % BLOCK_SWIZZLE_SIZE)
            )
            scheduler_n_local = scheduler_cluster_m % BLOCK_SWIZZLE_SIZE
            scheduler_swizzle_group_n = (
                scheduler_cluster_n // BLOCK_SWIZZLE_SIZE
            )
            scheduler_swizzled_m_candidate = cutlass.Int32(
                cutlass.select_(
                    (scheduler_swizzle_group_n % 2) == 0,
                    scheduler_m_local,
                    swizzle_m_bound - scheduler_m_local - 1,
                )
            )
            scheduler_swizzled_n_candidate = (
                scheduler_n_local
                + scheduler_swizzle_group_n * BLOCK_SWIZZLE_SIZE
            )
            scheduler_swizzled_cluster_m = cutlass.Int32(
                cutlass.select_(
                    scheduler_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        scheduler_cluster_n < swizzle_n_bound,
                        scheduler_swizzled_m_candidate,
                        scheduler_cluster_m,
                    ),
                    scheduler_cluster_m,
                )
            )
            scheduler_swizzled_cluster_n = cutlass.Int32(
                cutlass.select_(
                    scheduler_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        scheduler_cluster_n < swizzle_n_bound,
            ...
            scheduler_work_n = (
                scheduler_swizzled_cluster_n * CLUSTER_SHAPE_MN[1]
                + cta_n_in_cluster
            )
        ...
        if warp_idx == MMA_WARP_ID:
            ...
            mma_m_local = (
                mma_cluster_m // BLOCK_SWIZZLE_SIZE
                + swizzle_m_size * (mma_cluster_n % BLOCK_SWIZZLE_SIZE)
            )
            mma_n_local = mma_cluster_m % BLOCK_SWIZZLE_SIZE
            mma_swizzle_group_n = mma_cluster_n // BLOCK_SWIZZLE_SIZE
            mma_swizzled_m_candidate = cutlass.Int32(
                cutlass.select_(
                    (mma_swizzle_group_n % 2) == 0,
                    mma_m_local,
                    swizzle_m_bound - mma_m_local - 1,
                )
            )
            mma_swizzled_n_candidate = (
                mma_n_local + mma_swizzle_group_n * BLOCK_SWIZZLE_SIZE
            )
            mma_swizzled_cluster_m = cutlass.Int32(
                cutlass.select_(
                    mma_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        mma_cluster_n < swizzle_n_bound,
                        mma_swizzled_m_candidate,
                        mma_cluster_m,
                    ),
                    mma_cluster_m,
                )
            )
            mma_swizzled_cluster_n = cutlass.Int32(
                cutlass.select_(
                    mma_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        mma_cluster_n < swizzle_n_bound,
            ...
            mma_work_n = (
                mma_swizzled_cluster_n * CLUSTER_SHAPE_MN[1] + cta_n_in_cluster
            )
            ...
            while mma_work_valid:
                ...
                next_n_local = next_cluster_m % BLOCK_SWIZZLE_SIZE
                next_swizzle_group_n = next_cluster_n // BLOCK_SWIZZLE_SIZE
                next_swizzled_m_candidate = cutlass.Int32(
                    cutlass.select_(
                        (next_swizzle_group_n % 2) == 0,
                        next_m_local,
                        swizzle_m_bound - next_m_local - 1,
                    )
                )
                next_swizzled_n_candidate = (
                    next_n_local + next_swizzle_group_n * BLOCK_SWIZZLE_SIZE
                )
                next_swizzled_cluster_m = cutlass.Int32(
                    cutlass.select_(
                        next_cluster_m < swizzle_m_bound,
                        cutlass.select_(
                            next_cluster_n < swizzle_n_bound,
                            next_swizzled_m_candidate,
                            next_cluster_m,
                        ),
                        next_cluster_m,
                    )
                )
                next_swizzled_cluster_n = cutlass.Int32(
                    cutlass.select_(
                        next_cluster_m < swizzle_m_bound,
                        cutlass.select_(
                            next_cluster_n < swizzle_n_bound,
                ...
                next_work_n = (
                    next_swizzled_cluster_n * CLUSTER_SHAPE_MN[1]
                    + cta_n_in_cluster
                )
        ...
        if warp_idx < EPILOGUE_WARPS:
            ...
            epi_n_local = epi_cluster_m % BLOCK_SWIZZLE_SIZE
            epi_swizzle_group_n = epi_cluster_n // BLOCK_SWIZZLE_SIZE
            epi_swizzled_m_candidate = cutlass.Int32(
                cutlass.select_(
                    (epi_swizzle_group_n % 2) == 0,
                    epi_m_local,
                    swizzle_m_bound - epi_m_local - 1,
                )
            )
            epi_swizzled_n_candidate = (
                epi_n_local + epi_swizzle_group_n * BLOCK_SWIZZLE_SIZE
            )
            epi_swizzled_cluster_m = cutlass.Int32(
                cutlass.select_(
                    epi_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        epi_cluster_n < swizzle_n_bound,
                        epi_swizzled_m_candidate,
                        epi_cluster_m,
                    ),
                    epi_cluster_m,
                )
            )
            epi_swizzled_cluster_n = cutlass.Int32(
                cutlass.select_(
                    epi_cluster_m < swizzle_m_bound,
                    cutlass.select_(
                        epi_cluster_n < swizzle_n_bound,
            ...
            epi_work_n = (
                epi_swizzled_cluster_n * CLUSTER_SHAPE_MN[1] + cta_n_in_cluster
            )
            ...
            while epi_work_valid:
                ...
                epi_work_n = (
                    epi_swizzled_cluster_n * CLUSTER_SHAPE_MN[1]
                    + cta_n_in_cluster
                )
...
def run_matmul_local(
    ...
    try:
        ...
        raise RuntimeError(
            "kernel9_thread_block_swizzle requires the CuTeDSL runtime, "
    ...
    if err != cu_driver.CUresult.CUDA_SUCCESS or device_count < 1:
        raise RuntimeError("A GPU is required to run kernel9_thread_block_swizzle.")
    ...
    result: dict[str, Any] = {
        ...
        "kernel": "kernel9_thread_block_swizzle",
        "implementation": "cutedsl_blackwell_clc_persistent_block_swizzle",
        "operation": "cute.gemm",
        "dim": dim,
        "seed": seed,
...
image = (
    ...
    .add_local_file(__file__, remote_path="/root/kernel_9_thread_block_swizzle.py")
)

app = modal.App("blackwell-matmul-kernel-9-thread-block-swizzle", image=image)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run Cutedsl kernel 9 Blackwell matmul benchmark"
    )
